    ...
    for (uint32_t n = 0; n < tree->triangle_count; ++n) {
        ...
        for (int py = top; py <= bottom; ++py) {
            for (int px = left; px <= right; ++px) {
                const float b = ((px - x[0]) * (y[2] - y[0]) - (py - y[0]) * (x[2] - x[0])) / area;

                const float c = ((x[1] - x[0]) * (py - y[0]) - (y[1] - y[0]) * (px - x[0])) / area;
...
static bool compress(BEAM_GRID *grid, const uint32_t *prefix, uint32_t x, uint32_t y, uint32_t z, uint32_t side) {
    ...
    return half && compress(grid, prefix, x, y, z, half) && compress(grid, prefix, x + half, y, z, half) && compress(grid, prefix, x, y + half, z, half) &&
           compress(grid, prefix, x + half, y + half, z, half);
}

static void prefix_slice(uint32_t *prefix, const unsigned char *samples, uint32_t width, uint32_t height) {
    ...
    for (uint32_t y = 0; y < height; ++y) {
        ...
        for (uint32_t x = 0; x < width; ++x) {
            ...
            prefix[(size_t)(y + 1u) * stride + x + 1u] = prefix[(size_t)y * stride + x + 1u] + row;
        }
    }
}
...
bool beam_build(BEAM_GRID *grid, const MESH *scene, const BVH *tree, VEC3 sun_direction) {
    ...
    grid->origin = min;
    grid->step = v3(fmaxf((max.x - min.x) / grid->width, 0.001f), fmaxf((max.y - min.y) / grid->height, 0.001f), fmaxf(z_span / grid->depth, 0.001f));
    ...
    SDL_Log("B: sun beam grid %ux%ux%u, %.3f world units/slice", grid->width, grid->height, grid->depth, grid->step.z);

    raster_depth(depth, grid, tree, u, v, sun);

    for (uint32_t y = 0; y < grid->height; y += BEAM_TILE)
        for (uint32_t x = 0; x < grid->width; x += BEAM_TILE)
            tiles[x / BEAM_TILE + (size_t)(grid->width / BEAM_TILE) * (y / BEAM_TILE)] = measure_depth_tile(depth, grid, x, y);
    ...
    for (uint32_t z = 0; z < grid->depth && good; ++z) {
        prefix_slice(prefix, samples + (size_t)grid->width * grid->height * z, grid->width, grid->height);
