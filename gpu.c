...
static bool create_pipeline_layout(renderer *r, NriPipelineLayout **out,
...
static bool create_surface_layout(renderer *r) {
    ...
    return create_pipeline_layout(r, &r->surface_layout, sets, counts,
...
static bool create_line_layout(renderer *r) {
    ...
    return create_pipeline_layout(r, &r->line_layout, sets, counts,
                       NriStageBits_VERTEX_SHADER | NriStageBits_FRAGMENT_SHADER);
}
...
static bool create_bake_layout(renderer *r) {
    ...
    return create_compute_layout(
        r, &r->bake_layout, src, 10,
...
static bool track_descriptor_array(renderer *r, NriDescriptor ***items,
...
static bool track_buffer_array(renderer *r, NriBuffer ***items,
                               uint32_t *count, uint32_t *capacity,
                               NriBuffer *buffer) {
...
static bool track_descriptor(renderer *r, NriDescriptor *descriptor) {
    if (r->active_frame) {
        ...
        return track_descriptor_array(
            r, &frame->temporary_descriptors,
            &frame->temporary_descriptor_num, &frame->temporary_descriptor_cap,
            descriptor);
    }
    return track_descriptor_array(
        r, &r->temporary_descriptors,
        &r->temporary_descriptor_num, &r->temporary_descriptor_cap,
        descriptor);
}
...
static bool track_buffer(renderer *r, NriBuffer *buffer) {
    if (r->active_frame) {
        frame_context *frame = r->active_frame;
        return track_buffer_array(
            r, &frame->temporary_buffers,
            &frame->temporary_buffer_num, &frame->temporary_buffer_cap,
            buffer);
    }
    return track_buffer_array(
        r, &r->temporary_buffers,
        &r->temporary_buffer_num, &r->temporary_buffer_cap,
        buffer);
}
...
static void clear_frame_temporary(renderer *r, frame_context *frame) {
    ...
    if (frame->descriptor_pool)
        r->core.ResetDescriptorPool(frame->descriptor_pool);
}
...
static bool create_frame_contexts(renderer *r) {
    ...
    if (r->core.CreateFence(r->device, 0, &r->frame_fence) != NriResult_SUCCESS)
        return false;

    for (uint32_t i = 0; i < FRAME_QUEUE_DEPTH; ++i) {
        frame_context *frame = &r->frame_contexts[i];
        if (!create_descriptor_pool_object(r, &frame->descriptor_pool) ||
            r->core.CreateCommandAllocator(
                r->graphics_queue, &frame->allocator) != NriResult_SUCCESS ||
            r->core.CreateCommandBuffer(
                frame->allocator, &frame->command_buffer) != NriResult_SUCCESS)
            return false;
    }
    return true;
}
    ...
    if (r->frame_contexts) {
        for (uint32_t i = 0; i < FRAME_QUEUE_DEPTH; ++i) {
            ...
            if (frame->command_buffer)
                r->core.DestroyCommandBuffer(frame->command_buffer);
            if (frame->allocator)
                r->core.DestroyCommandAllocator(frame->allocator);
            if (frame->descriptor_pool)
                r->core.DestroyDescriptorPool(frame->descriptor_pool);
...
static bool begin_frame_commands(renderer *r, frame_context **out_frame,
                                 NriCommandBuffer **out_command_buffer) {
    if (!r || !r->frame_contexts || !r->frame_fence || !out_frame ||
        !out_command_buffer)
        return false;

    const uint64_t wait_value = r->frame_index >= FRAME_QUEUE_DEPTH
        ? 1u + r->frame_index - FRAME_QUEUE_DEPTH : 0u;
    ...
    if (r->core.BeginCommandBuffer(
            frame->command_buffer, frame->descriptor_pool) != NriResult_SUCCESS) {
        r->active_frame = NULL;
        return false;
    }
...
static void abort_frame_commands(renderer *r, frame_context *frame) {
    ...
    if (r->core.CreateCommandAllocator(
            r->graphics_queue, &frame->allocator) == NriResult_SUCCESS)
        r->core.CreateCommandBuffer(frame->allocator, &frame->command_buffer);
}
...
static bool bind_descriptor_set(renderer *r, NriCommandBuffer *cmd, NriPipelineLayout *layout,
                     NriBindPoint point, uint32_t set_index,
                     NriDescriptor *const *descriptors, uint32_t count) {
    NriDescriptorSet *set = NULL;
    NriDescriptorPool *pool = r->active_frame
        ? r->active_frame->descriptor_pool : r->descriptor_pool;
    if (r->core.AllocateDescriptorSets(pool, layout, set_index,
...
static bool bind_uniform_data(renderer *r, NriCommandBuffer *cmd, NriPipelineLayout *layout,
...
static bool transition_texture(renderer *r, NriCommandBuffer *cmd, NriTexture *texture,
                               NriAccessBits access, NriLayout layout,
                               NriStageBits stages);

static bool bind_bake_resources(renderer *r, NriCommandBuffer *cmd,
                                NriTexture *source, NriTexture *destination,
                                const bake_uniforms *uniforms, size_t size) {
    if (!r || !cmd || !source || !destination || !r->lightmap_sampler ||
        !r->bvh_node_buffer || !r->bvh_triangle_buffer ||
        !r->lightmap_sample_buffer || !r->lightmap_probe_buffer)
        return false;

    NriTexture *direct = r->lightmap_direct && r->lightmap_direct != destination
        ? r->lightmap_direct : source;
    if (!transition_texture(r, cmd, source, NriAccessBits_SHADER_RESOURCE,
                            NriLayout_SHADER_RESOURCE, NriStageBits_COMPUTE_SHADER) ||
        (direct != source && !transition_texture(
            r, cmd, direct, NriAccessBits_SHADER_RESOURCE,
            NriLayout_SHADER_RESOURCE, NriStageBits_COMPUTE_SHADER)) ||
        !transition_texture(r, cmd, destination,
                            NriAccessBits_SHADER_RESOURCE_STORAGE,
                            NriLayout_SHADER_RESOURCE_STORAGE,
                            NriStageBits_COMPUTE_SHADER))
        return false;

    NriBuffer *patch_map = r->lightmap_patch_map_buffer
        ? r->lightmap_patch_map_buffer : r->lightmap_sample_buffer;
    NriBuffer *patch_anchors = r->lightmap_patch_anchor_buffer
        ? r->lightmap_patch_anchor_buffer : r->lightmap_sample_buffer;
    ...
    NriDescriptor *src[] = {
        ...
        r->lightmap_sampler,
        create_buffer_view(r, r->bvh_node_buffer,
                    NriBufferView_STRUCTURED_BUFFER, sizeof(bvh_node)),
        create_buffer_view(r, r->bvh_triangle_buffer,
                    NriBufferView_STRUCTURED_BUFFER, sizeof(bvh_triangle)),
        create_buffer_view(r, r->lightmap_sample_buffer,
                    NriBufferView_STRUCTURED_BUFFER, sizeof(lmap_sample)),
        create_buffer_view(r, r->lightmap_probe_buffer,
                    NriBufferView_STRUCTURED_BUFFER, sizeof(probe)),
        create_buffer_view(r, patch_map,
                    NriBufferView_STRUCTURED_BUFFER, sizeof(Uint32)),
        create_buffer_view(r, patch_anchors,
                    NriBufferView_STRUCTURED_BUFFER, sizeof(Uint32[4]))
    };
    NriDescriptor *dst = create_texture_view(
        r, destination, NriTextureView_STORAGE_TEXTURE);

    return bind_descriptor_set(r, cmd, r->bake_layout, NriBindPoint_COMPUTE, 0, src, 10) &&
           bind_descriptor_set(r, cmd, r->bake_layout, NriBindPoint_COMPUTE, 1, &dst, 1) &&
           bind_uniform_data(r, cmd, r->bake_layout, NriBindPoint_COMPUTE, 2,
                        uniforms, size);
}

static bool bind_probe_resources(renderer *r, NriCommandBuffer *cmd,
                                 NriBuffer *input, NriBuffer *nodes, NriBuffer *triangles,
                                 NriBuffer *output, const bake_uniforms *uniforms,
                                 size_t size) {
...
static bool read_buffer(renderer *r, NriBuffer *output, probe_grid *grid,
...
static bool bind_fx_resources(renderer *r, NriCommandBuffer *cmd,
                              NriPipeline *pipeline, NriTexture *source,
                              NriTexture *destination, NriDescriptor *sampler_desc,
...
static bool bind_volume_resources(renderer *r, NriCommandBuffer *cmd,
                                  NriTexture *normal, NriDescriptor *sampler_desc,
                                  NriBuffer *probes, NriBuffer *beams, NriTexture *output,
                                  const void *uniforms, uint32_t size) {
...
static bool bind_volume_compose_resources(renderer *r, NriCommandBuffer *cmd,
                                          NriTexture *hdr, NriTexture *volume, NriTexture *normal,
...
static bool bind_surface_resources(renderer *r, NriCommandBuffer *cmd,
                                   const render_material *material, NriTexture *lightmap,
...
static bool texture_barrier(renderer *r, NriCommandBuffer *cmd, NriTexture *texture,
...
static bool begin_scene_rendering(renderer *r, NriCommandBuffer *cmd,
                                  NriTexture *hdr, NriTexture *normal, NriTexture *depth,
...
static bool begin_compose_rendering(renderer *r, NriCommandBuffer *cmd, NriTexture *swap,
                                    NriTexture *hdr, NriTexture *ao, NriTexture *bloom,
                                    NriTexture *lut, NriDescriptor *sampler_desc,
                                    const void *uniforms, size_t size) {
...
static bool submit_frame(renderer *r, frame_context *frame,
                         NriCommandBuffer *cmd, uint32_t index) {
    bool good = frame && texture_barrier(
        r, cmd, r->swapchain_textures[index],
        (NriAccessLayoutStage){.access = NriAccessBits_COLOR_ATTACHMENT,
    ...
    if (good)
        good = r->swapchain_api.QueuePresent(
            r->swapchain, r->swapchain_frames[index].release,
            frame_value) == NriResult_SUCCESS;
...
static NriTexture *create_texture(renderer *r, NriFormat format,
...
static void destroy_upload_context(renderer *r) {
    ...
    for (uint32_t i = 0; i < UPLOAD_RING_SIZE; ++i) {
        ...
        if (slot->command_buffer)
            r->core.DestroyCommandBuffer(slot->command_buffer);
        if (slot->allocator)
            r->core.DestroyCommandAllocator(slot->allocator);
        if (slot->staging)
            r->core.DestroyBuffer(slot->staging);
    }
    if (upload->fence)
        r->core.DestroyFence(upload->fence);
...
static bool create_upload_context(renderer *r) {
    ...
    if (r->core.CreateFence(
            r->device, 0u, &upload->fence) != NriResult_SUCCESS)
        goto fail;
    ...
    for (uint32_t i = 0; i < UPLOAD_RING_SIZE; ++i) {
        ...
        if (r->core.CreateCommittedBuffer(
                r->device,
                NriMemoryLocation_HOST_UPLOAD,
                0.0f,
                &staging_desc,
                &slot->staging) != NriResult_SUCCESS ||
            r->core.CreateCommandAllocator(
                r->graphics_queue,
                &slot->allocator) != NriResult_SUCCESS ||
            r->core.CreateCommandBuffer(
                slot->allocator,
                &slot->command_buffer) != NriResult_SUCCESS)
            goto fail;
    }
...
static bool upload_wait_slot(renderer *r, upload_slot *slot) {
    ...
    while (r->core.GetFenceValue(r->upload->fence) < slot->fence_value) {
        if (!logged && SDL_GetTicks() - started >= UPLOAD_SLOW_LOG_MS) {
            SDL_Log("GPU upload chunk is still pending after %u ms; continuing without NRI's hard fence timeout",
                    UPLOAD_SLOW_LOG_MS);
            logged = true;
        }
        SDL_Delay(1u);
...
static bool upload_begin_slot(renderer *r, upload_slot **out) {
    ...
    if (r->core.BeginCommandBuffer(
            slot->command_buffer, NULL) != NriResult_SUCCESS)
        return false;
...
static bool upload_submit_slot(renderer *r, upload_slot *slot) {
    ...
    if (r->core.QueueSubmit(
            r->graphics_queue, &submit) != NriResult_SUCCESS)
        return false;
...
static NriAccessStage uploaded_buffer_state(NriBufferUsageBits usage) {
    ...
    if (usage & NriBufferUsageBits_SHADER_RESOURCE) {
        ...
        state.stages |= NriStageBits_COMPUTE_SHADER |
...
static NriBuffer *upload_buffer(renderer *r, NriBufferUsageBits usage,
    ...
    size_t offset = 0u;
    while (offset < bytes) {
        const size_t chunk = bytes - offset > UPLOAD_CHUNK_BYTES
            ? UPLOAD_CHUNK_BYTES : bytes - offset;
        ...
        if (!offset) {
            ...
            r->core.CmdBarrier(
                slot->command_buffer,
                &(NriBarrierDesc){.buffers = &barrier, .bufferNum = 1u});
        }

        r->core.CmdCopyBuffer(
            slot->command_buffer,
            buffer, offset,
            slot->staging, 0u,
            chunk);
...
static bool upload_texture_data(renderer *r, NriTexture *texture,
                                const void *data,
                                uint32_t row_pitch,
                                ...
                                NriLayout layout,
                                NriStageBits stages) {
    if (!r || !texture || !data || !row_pitch || !slice_pitch ||
    ...
    const uint32_t row_alignment =
        device->memoryAlignment.uploadBufferTextureRow;
    const uint32_t slice_alignment =
        device->memoryAlignment.uploadBufferTextureSlice;
    const uint64_t aligned_row = upload_align(row_pitch, row_alignment);
    const uint32_t row_count = slice_pitch / row_pitch;
    if (!desc || !row_count || aligned_row > UPLOAD_CHUNK_BYTES)
        return false;
    ...
    while (first_row < row_count) {
        ...
        uint64_t staging_bytes = upload_align(
            aligned_row * rows, slice_alignment);
        while (rows > 1u && staging_bytes > UPLOAD_CHUNK_BYTES) {
            --rows;
            staging_bytes = upload_align(
                aligned_row * rows, slice_alignment);
        }
        ...
        upload_slot *slot = NULL;
        if (!upload_begin_slot(r, &slot)) goto fail;
        Uint8 *mapped = r->core.MapBuffer(
            slot->staging, 0u, staging_bytes);
        if (!mapped) {
            (void)r->core.EndCommandBuffer(slot->command_buffer);
            goto fail;
        }
        ...
        for (uint32_t row = 0; row < rows; ++row)
            memcpy(mapped + (size_t)row * aligned_row,
                   source + (size_t)(first_row + row) * row_pitch,
                   row_pitch);
        r->core.UnmapBuffer(slot->staging);

        if (!first_row && !texture_barrier(
                r,
                slot->command_buffer,
                texture,
                (NriAccessLayoutStage){0},
                copy_state))
            goto fail;
        ...
        r->core.CmdUploadBufferToTexture(
            slot->command_buffer,
            texture,
            &region,
            slot->staging,
        ...
        if (first_row + rows == row_count && !texture_barrier(
                r,
                slot->command_buffer,
                texture,
                copy_state,
...
static NriTexture *pixel_texture(renderer *r, Uint8 red, Uint8 green,
                              Uint8 blue, Uint8 alpha) {
    const Uint8 pixels[4] = {red, green, blue, alpha};

    NriTexture *result = create_texture(
        r,
...
static bool ensure_depth_texture(renderer *r, Uint32 width, Uint32 height) {
    ...
    r->depth_texture = create_texture(
        r,
        r->depth_format,
...
static NriTexture *resolve_texture(renderer *r, const gltf_scene *visual,
                                int32_t texture_index, NriTexture *fallback) {
    if (texture_index < 0 ||
...
bool upload_scene(renderer *r, const gltf_scene *visual) {
    ...
    const uint64_t vertex_bytes =
        (uint64_t)r->vertex_count * sizeof(*r->vertices);
...
static NriTexture *create_lightmap_texture(renderer *r,
                                        Uint32 width, Uint32 height) {
...
NriTexture *upload_lightmap(renderer *r, const cached_lightmap *cached) {
    ...
    NriTexture *result = create_lightmap_texture(
...
static bool read_rgba16f_texture(renderer *r, NriTexture *texture,
                                  Uint32 width, Uint32 height,
                                  Uint8 **pixels) {
    if (!r || !r->device || !texture || !width || !height || !pixels)
        return false;
    *pixels = NULL;
    ...
    const uint64_t row_pitch =
        ((uint64_t)row_bytes + row_alignment - 1u) / row_alignment * row_alignment;
    const uint64_t slice_bytes = row_pitch * height;
    const uint64_t staging_bytes =
        (slice_bytes + slice_alignment - 1u) / slice_alignment * slice_alignment;
    ...
    if (r->core.CreateCommittedBuffer(
            r->device, NriMemoryLocation_HOST_READBACK, 1.0f,
            &desc, &readback) != NriResult_SUCCESS)
        return false;
    ...
    if (good) {
        ...
        good = transition_texture(
            r, cmd, texture, NriAccessBits_COPY_SOURCE,
            NriLayout_COPY_SOURCE, NriStageBits_COPY);
        if (good) {
            r->core.CmdReadbackTextureToBuffer(
                cmd, readback, &layout, texture, &region);
            good = submit_commands(r, allocator, cmd);
            allocator = NULL;
            cmd = NULL;
        }
    ...
    if (data) {
        for (uint32_t y = 0; y < height; ++y)
            memcpy(data + (size_t)y * row_bytes,
                   mapped + (size_t)y * row_pitch, row_bytes);
    }
...
bool download_lightmap(renderer *r, cached_lightmap *out) {
    ...
    Uint8 *pixels = NULL;
    if (!read_rgba16f_texture(
            r, r->lightmap_texture,
...
NriBuffer *upload_probes(renderer *r, const probe_grid *grid) {
    ...
    return upload_buffer(
        ...
        NriBufferUsageBits_SHADER_RESOURCE,
        grid->probes,
        count * sizeof(probe), sizeof(probe));
}
...
NriBuffer *upload_beams(renderer *r, const beam_grid *grid) {
    ...
    NriBuffer *buffer = upload_buffer(
        r,
        NriBufferUsageBits_SHADER_RESOURCE,
        data,
...
static bake_uniforms bake_data(renderer *r, Uint32 phase, Uint32 iteration,
    ...
    return (bake_uniforms){
        ...
        .bake_params = {r->bake_epsilon, 0.72f,
                        1.0f, (float)r->lightmap_min_samples},
        .probe_origin_spacing = {
            r->lightmap_probe_origin.x,
            r->lightmap_probe_origin.y,
            r->lightmap_probe_origin.z,
            r->lightmap_probe_spacing
        },
        .probe_dims_mode = {
            r->lightmap_probe_count_x,
            r->lightmap_probe_count_y,
            r->lightmap_probe_count_z,
            0u
        }
    };
}
...
static bool record_bake_pass(renderer *r, NriCommandBuffer *cmd,
                             ...
                             Uint32 item_count, Uint32 batch_count) {
    ...
    const bake_uniforms uniforms = bake_data(
        r, phase, iteration, item_count, dispatch_width, batch_count);
    if (!bind_bake_resources(
            r, cmd, source, destination,
...
static float decode_half(Uint16 h) {
    ...
    float value = exponent == 0u ? ldexpf((float)mantissa, -24) :
        exponent == 31u ? INFINITY :
        ldexpf((float)(1024u + mantissa), (int)exponent - 25);
...
static float direct_luma(const Uint8 *pixels, Uint32 pixel) {
    ...
    return decode_half(channels[0]) * 0.2126f +
           decode_half(channels[1]) * 0.7152f +
           decode_half(channels[2]) * 0.0722f;
}

static bool build_lightmap_patches(renderer *r, const lightmap *lm) {
    Uint8 *pixels = NULL;
    if (!read_rgba16f_texture(
            r, r->lightmap_direct, lm->width, lm->height, &pixels))
        return false;
        ...
        for (Uint32 x = 0; x + 3u < lm->width; x += 4u) {
            ...
            for (Uint32 dy = 0; dy < 4u && smooth; ++dy) {
                for (Uint32 dx = 0; dx < 4u; ++dx) {
                    ...
                    float dot = base->normal[0] * sample->normal[0] +
                                base->normal[1] * sample->normal[1] +
                                base->normal[2] * sample->normal[2];
                    float plane =
                        (sample->position[0] - base->position[0]) * base->normal[0] +
                        (sample->position[1] - base->position[1]) * base->normal[1] +
                        (sample->position[2] - base->position[2]) * base->normal[2];
                    if (sample->normal[3] != base->normal[3] || dot < 0.995f ||
                        fabsf(plane) > 0.025f) {
                        smooth = false;
                        break;
                    }
            ...
            if (!smooth || !isfinite(hi) || hi - lo > 0.06f + 0.1f * hi)
                continue;
            ...
            for (Uint32 j = 0; j < 4u; ++j) {
                ...
                anchors[tile][j] =
                    (y + corners[j] / 4u) * lm->width + x + corners[j] % 4u;
            }
            for (Uint32 j = 0; j < 16u; ++j) {
                mapping[indices[j]] = tile;
                if (j != 0u && j != 3u && j != 12u && j != 15u)
                    keep[indices[j]] = 0u;
            }
            reduced += 12u;
            active_tiles++;
    ...
    if (reduced) {
        ...
        if (sparse) {
            ...
            sample_buffer = upload_buffer(
                r, NriBufferUsageBits_SHADER_RESOURCE,
                sparse, (size_t)count * sizeof(*sparse), sizeof(lmap_sample));
            map_buffer = upload_buffer(
                r, NriBufferUsageBits_SHADER_RESOURCE,
                mapping, (size_t)lm->sample_count * sizeof(*mapping), sizeof(Uint32));
            anchor_buffer = upload_buffer(
                r, NriBufferUsageBits_SHADER_RESOURCE,
                anchors, tile_count * sizeof(*anchors), sizeof(Uint32[4]));
        }
        free(sparse);
        ...
        if (ok) {
            ...
            SDL_Log(
                "B: lightmap 4x4 patches: %u chart-safe tiles, %u/%u indirect texels",
                active_tiles, count, lm->sample_count);
        } else {
...
static bool bake_lightmap_once(renderer *r, const lightmap *lm) {
    ...
    if (!record_bake_pass(r, cmd, r->lightmap_scratch,
                          r->lightmap_texture, PHASE_CLEAR, 0, pixels, 0) ||
        !record_bake_pass(r, cmd, r->lightmap_texture,
                          r->lightmap_scratch, PHASE_CLEAR, 0, pixels, 0) ||
        !record_bake_pass(r, cmd, r->lightmap_scratch,
                          r->lightmap_direct, PHASE_CLEAR, 0, pixels, 0) ||
        !record_bake_pass(r, cmd, r->lightmap_scratch,
                          r->lightmap_direct, PHASE_DIRECT, 0,
                          r->lightmap_sample_count, 0)) {
        abort_commands(r, allocator, cmd);
        return false;
    }
    ...
    for (Uint32 first = 0; first < r->bake_target_samples;
        ...
        if (r->lightmap_trace_count &&
            !submit_trace_batch(r, first, count, r->lightmap_trace_count))
            return false;
        bake_progress(r, "surface lightmap", first + count,
    ...
    if (r->lightmap_full_sample_buffer) {
        ...
        if (!record_bake_pass(r, cmd, r->lightmap_texture,
                              r->lightmap_scratch, PHASE_CLEAR, 0, pixels, 0) ||
            !record_bake_pass(r, cmd, r->lightmap_texture,
                              r->lightmap_scratch, PHASE_RECONSTRUCT, 0,
                              r->lightmap_sample_count, 0)) {
            abort_commands(r, allocator, cmd);
            return false;
        }
    ...
    if (!record_bake_pass(r, cmd, r->lightmap_texture,
                          r->lightmap_scratch, PHASE_COMBINE, 0, pixels, 0)) {
        abort_commands(r, allocator, cmd);
        return false;
    }
    swap_lightmaps(r);
    ...
    if (!record_bake_pass(r, cmd, r->lightmap_texture,
                          r->lightmap_scratch, PHASE_FILTER, 0, pixels, 0)) {
        abort_commands(r, allocator, cmd);
        return false;
    }
    swap_lightmaps(r);
    ...
    for (Uint32 i = 0; i < BAKE_DILATION_PASSES; ++i) {
        if (!record_bake_pass(r, cmd, r->lightmap_texture,
                              r->lightmap_scratch, PHASE_DILATE, 0, pixels, 0)) {
            abort_commands(r, allocator, cmd);
            return false;
        }
        swap_lightmaps(r);
...
bool bake_lightmap(renderer *r, const bvh *tree, const lightmap *lm,
    ...
    if (!r || !r->device || !tree || !tree->node_count || !lm ||
        ...
        !r->lightmap_sampler || !probes || !probes->probes || !probes->spacing ||
        !probes->count_x || !probes->count_y || !probes->count_z)
        return false;
    ...
    if (!r->lightmap_texture || !r->lightmap_scratch || !r->lightmap_direct)
        return false;
    ...
    r->bake_pipeline = compile_compute(
        &r->core, r->device, r->bake_layout,
        "shaders/compute.hlsl", "lightmap_cs", "BUILD_LIGHTMAP_CS");
...
static probe_wavefront_buffer probe_wavefront_uploaded(
    renderer *r, const void *data, uint64_t bytes, uint32_t stride) {
    probe_wavefront_buffer result = {0};
    if (!data || !bytes || bytes > SIZE_MAX) return result;
    result.buffer = upload_buffer(
        r, NriBufferUsageBits_SHADER_RESOURCE,
        data, (size_t)bytes, stride);
    if (result.buffer) {
...
static probe_wavefront_buffer probe_wavefront_storage(
    renderer *r, uint64_t bytes, uint32_t stride) {
    probe_wavefront_buffer result = {0};
    ...
    const NriBufferDesc desc = {
        ...
        .usage = NriBufferUsageBits_SHADER_RESOURCE |
                 NriBufferUsageBits_SHADER_RESOURCE_STORAGE
    };
    if (r->core.CreateCommittedBuffer(
            r->device, NriMemoryLocation_DEVICE, 1.0f,
            &desc, &result.buffer) == NriResult_SUCCESS)
        result.stride = stride;
    return result;
}

static void probe_wavefront_release_buffer(renderer *r,
                                           probe_wavefront_buffer *buffer) {
    if (!buffer) return;
    release_buffer(r, buffer->buffer);
    *buffer = (probe_wavefront_buffer){0};
}

static bool probe_wavefront_layout(renderer *r, NriPipelineLayout **layout,
                                   uint8_t reads, uint8_t writes) {
    NriDescriptorType read_types[4] = {0};
    NriDescriptorType write_types[3] = {0};
    static const NriDescriptorType uniform[] = {
        NriDescriptorType_CONSTANT_BUFFER
    };
    ...
    const NriDescriptorType *sets[4] = {
        reads ? read_types : NULL,
        writes ? write_types : NULL,
        uniform,
        NULL
    };
    const uint8_t counts[4] = {reads, writes, 1u, 0u};
    return create_pipeline_layout(r, layout, sets, counts,
                       NriStageBits_COMPUTE_SHADER);
}

static bool probe_wavefront_stage_init(renderer *r,
                                       probe_wavefront_stage *stage,
                                       const char *entrypoint,
                                       const char *define,
                                       uint8_t reads, uint8_t writes) {
    if (!probe_wavefront_layout(r, &stage->layout, reads, writes))
        return false;
    stage->pipeline = compile_compute(
        &r->core, r->device, stage->layout,
        "shaders/probe_wavefront.hlsl", entrypoint, define);
    stage->read_count = reads;
    stage->write_count = writes;
    return stage->pipeline != NULL;
}

static void probe_wavefront_stage_deinit(renderer *r,
                                         probe_wavefront_stage *stage) {
    if (!stage) return;
...
static bool probe_wavefront_pipelines_init(renderer *r,
                                           probe_wavefront_pipelines *p) {
    memset(p, 0, sizeof(*p));
    return probe_wavefront_stage_init(
               r, &p->prepare, "probe_prepare_cs", "BUILD_PROBE_PREP_CS", 2, 2) &&
           probe_wavefront_stage_init(
               r, &p->reset, "probe_reset_cs", "BUILD_PROBE_RESET_CS", 0, 3) &&
           probe_wavefront_stage_init(
               r, &p->validate, "probe_validate_cs", "BUILD_PROBE_VALIDATE_CS", 3, 1) &&
           probe_wavefront_stage_init(
               r, &p->primary, "probe_primary_cs", "BUILD_PROBE_PRIMARY_CS", 4, 3) &&
           probe_wavefront_stage_init(
               r, &p->bounce, "probe_bounce_cs", "BUILD_PROBE_BOUNCE_CS", 4, 3) &&
           probe_wavefront_stage_init(
               r, &p->reduce, "probe_reduce_cs", "BUILD_PROBE_REDUCE_CS", 1, 3);
}

static void probe_wavefront_pipelines_deinit(renderer *r,
                                             probe_wavefront_pipelines *p) {
...
static bool probe_wavefront_transition(
    renderer *r, NriCommandBuffer *cmd,
    probe_wavefront_buffer *const *reads, uint8_t read_count,
    ...
    for (uint8_t i = 0; i < read_count; ++i) {
        ...
        barriers[count++] = (NriBufferBarrierDesc){
            ...
            .after = {.access = NriAccessBits_SHADER_RESOURCE,
                      .stages = NriStageBits_COMPUTE_SHADER}
        };
    ...
    if (count)
        r->core.CmdBarrier(cmd, &(NriBarrierDesc){
            .buffers = barriers, .bufferNum = count
        });
    return true;
}
...
static bool probe_wavefront_dispatch(
    ...
    uint32_t groups_x) {
    if (!r || !cmd || !stage || !stage->pipeline || !stage->layout ||
        !uniforms || !groups_x)
        return false;
    if (!probe_wavefront_transition(
            r, cmd, reads, stage->read_count,
            writes, stage->write_count))
        return false;

    if (stage->read_count) {
        NriDescriptor *descriptors[4] = {0};
        for (uint8_t i = 0; i < stage->read_count; ++i)
            descriptors[i] = create_buffer_view(
                r, reads[i]->buffer,
                NriBufferView_STRUCTURED_BUFFER, reads[i]->stride);
        if (!bind_descriptor_set(r, cmd, stage->layout, NriBindPoint_COMPUTE,
                      0, descriptors, stage->read_count))
            return false;
    }
    if (stage->write_count) {
        ...
        for (uint8_t i = 0; i < stage->write_count; ++i)
            descriptors[i] = create_buffer_view(
                r, writes[i]->buffer,
                NriBufferView_STORAGE_STRUCTURED_BUFFER, writes[i]->stride);
        if (!bind_descriptor_set(r, cmd, stage->layout, NriBindPoint_COMPUTE,
                      1, descriptors, stage->write_count))
            return false;
    }
    if (!bind_uniform_data(r, cmd, stage->layout, NriBindPoint_COMPUTE,
                      2, uniforms, sizeof(*uniforms)))
        return false;
...
static probe_wavefront_uniforms probe_wavefront_data(
    ...
    uint32_t probe_count, uint32_t sample_offset,
    uint32_t block_samples, uint32_t bounce_index) {
    const vec3 sun = v3_normalize(v3(0.38f, 0.30f, 0.32f));
    const bvh_node *root = &tree->nodes[0];
    float scene_scale = fmaxf(
        root->max[0] - root->min[0],
        fmaxf(root->max[1] - root->min[1],
...
static NriBuffer *probe_wavefront_readback_buffer(renderer *r, uint64_t bytes) {
    ...
    if (r->core.CreateCommittedBuffer(
            r->device, NriMemoryLocation_HOST_READBACK, 1.0f,
            &desc, &buffer) != NriResult_SUCCESS)
        return NULL;
    return buffer;
}

static bool probe_wavefront_copy_to_readback(
    renderer *r, NriCommandBuffer *cmd,
    probe_wavefront_buffer *source, NriBuffer *destination,
    uint64_t bytes) {
    if (!source || !source->buffer || !destination) return false;
    const NriBufferBarrierDesc barrier = {
        .buffer = source->buffer,
        .before = {.access = source->access, .stages = source->stages},
        .after = {.access = NriAccessBits_COPY_SOURCE,
                  .stages = NriStageBits_COPY}
    };
    r->core.CmdBarrier(cmd, &(NriBarrierDesc){
        .buffers = &barrier, .bufferNum = 1
    });
...
bool bake_probe_grid_fast(renderer *r, probe_grid *grid,
    ...
    if (!r || !r->device || !grid || !grid->probes || !tree ||
        !tree->node_count || !tree->triangle_count || !beams ||
        !beams->shadow_depth)
        return false;

    const uint64_t probe_count64 =
    ...
    const size_t beam_count =
        (size_t)beams->width * beams->height * beams->depth;
    const size_t depth_count = (size_t)beams->width * beams->height;
    float *expanded = realloc(
        beam_data, (beam_count + depth_count) * sizeof(float));
    if (!expanded) {
        free(positions);
        free(beam_data);
        return false;
    ...
    beam_data = expanded;
    memcpy(beam_data + beam_count, beams->shadow_depth,
           depth_count * sizeof(float));

    probe_wavefront_buffer position_buffer = probe_wavefront_uploaded(
        r, positions, probe_count64 * sizeof(float[4]), sizeof(float[4]));
    probe_wavefront_buffer source_nodes = probe_wavefront_uploaded(
        r, tree->nodes, (uint64_t)tree->node_count * sizeof(*tree->nodes),
        sizeof(bvh_node));
    probe_wavefront_buffer source_triangles = probe_wavefront_uploaded(
        r, tree->triangles,
        (uint64_t)tree->triangle_count * sizeof(*tree->triangles),
        sizeof(bvh_triangle));
    probe_wavefront_buffer sun_beams = probe_wavefront_uploaded(
        r, beam_data, (beam_count + depth_count) * sizeof(float), sizeof(float));
    free(positions);
    free(beam_data);

    probe_wavefront_buffer packed_nodes = probe_wavefront_storage(
        r, (uint64_t)tree->node_count * PROBE_PACKED_NODE_BYTES,
        PROBE_PACKED_NODE_BYTES);
    probe_wavefront_buffer packed_triangles = probe_wavefront_storage(
        r, (uint64_t)tree->triangle_count * PROBE_PACKED_TRIANGLE_BYTES,
        PROBE_PACKED_TRIANGLE_BYTES);
    probe_wavefront_buffer states_a = probe_wavefront_storage(
        r, state_bytes, PROBE_RAY_STATE_BYTES);
    probe_wavefront_buffer states_b = probe_wavefront_storage(
        r, state_bytes, PROBE_RAY_STATE_BYTES);
    probe_wavefront_buffer results = probe_wavefront_storage(
        r, result_bytes, sizeof(float[4]));
    probe_wavefront_buffer accums = probe_wavefront_storage(
        r, accum_bytes, PROBE_ACCUM_BYTES);
    probe_wavefront_buffer coefficients = probe_wavefront_storage(
        r, output_bytes, sizeof(float[4]));
    probe_wavefront_buffer counters = probe_wavefront_storage(
        r, 4u * sizeof(uint32_t), sizeof(uint32_t));

    NriBuffer *counter_readback = probe_wavefront_readback_buffer(
        r, 4u * sizeof(uint32_t));
    NriBuffer *output_readback = probe_wavefront_readback_buffer(r, output_bytes);
    probe_wavefront_pipelines pipelines = {0};

    bool good = position_buffer.buffer && source_nodes.buffer &&
        source_triangles.buffer && sun_beams.buffer && packed_nodes.buffer &&
        packed_triangles.buffer && states_a.buffer && states_b.buffer &&
        results.buffer && accums.buffer && coefficients.buffer && counters.buffer &&
        counter_readback && output_readback &&
        probe_wavefront_pipelines_init(r, &pipelines);

    const uint32_t prep_groups = probe_wavefront_groups64(
    ...
    probe_wavefront_uniforms uniforms = probe_wavefront_data(
        tree, beams, probe_count, 0u, PROBE_BLOCK_SAMPLES, 0u);
    ...
    if (good) {
        ...
        if (good) {
            probe_wavefront_buffer *prepare_reads[] = {
                &source_nodes, &source_triangles
            };
            probe_wavefront_buffer *prepare_writes[] = {
                &packed_nodes, &packed_triangles
            };
            probe_wavefront_buffer *reset_writes[] = {
                &counters, &accums, &coefficients
            };
            probe_wavefront_buffer *validate_reads[] = {
                &position_buffer, &packed_nodes, &packed_triangles
            };
            probe_wavefront_buffer *validate_writes[] = {&accums};
            good = probe_wavefront_dispatch(
                       r, cmd, &pipelines.prepare,
                       prepare_reads, prepare_writes,
                       &uniforms, prep_groups) &&
                   probe_wavefront_dispatch(
                       r, cmd, &pipelines.reset,
                       NULL, reset_writes,
                       &uniforms, probe_groups) &&
                   probe_wavefront_dispatch(
                       r, cmd, &pipelines.validate,
                       validate_reads, validate_writes,
                       &uniforms, probe_groups);
    ...
    if (good && progress && !progress(0u, PROBE_MAX_SAMPLES, active))
        good = false;

    while (good && completed < PROBE_MAX_SAMPLES && active) {
        const uint32_t block =
            PROBE_MAX_SAMPLES - completed > PROBE_BLOCK_SAMPLES
                ? PROBE_BLOCK_SAMPLES : PROBE_MAX_SAMPLES - completed;
        uniforms = probe_wavefront_data(
            tree, beams, probe_count, completed, block, 0u);
        ...
        if (completed) {
            ...
            good = probe_wavefront_dispatch(
                r, cmd, &pipelines.reset, NULL, reset_writes,
                &uniforms, probe_groups);
        }

        probe_wavefront_buffer *primary_reads[] = {
            &position_buffer, &accums, &packed_nodes, &packed_triangles
        };
        probe_wavefront_buffer *primary_writes[] = {
            &results, &states_a, &counters
        };
        if (good)
            good = probe_wavefront_dispatch(
                r, cmd, &pipelines.primary,
        ...
        for (uint32_t bounce = 0; good && bounce < PROBE_MAX_BOUNCES; ++bounce) {
            ...
            probe_wavefront_buffer *bounce_reads[] = {
                &packed_nodes, &packed_triangles, input, &sun_beams
            };
            probe_wavefront_buffer *bounce_writes[] = {
                &results, output, &counters
            };
            good = probe_wavefront_dispatch(
                r, cmd, &pipelines.bounce,
                bounce_reads, bounce_writes,
                &uniforms, ray_groups);
        }

        uniforms.bounce_index = 0u;
        probe_wavefront_buffer *reduce_reads[] = {&results};
        probe_wavefront_buffer *reduce_writes[] = {
            &accums, &coefficients, &counters
        };
        if (good)
            good = probe_wavefront_dispatch(
                r, cmd, &pipelines.reduce,
                reduce_reads, reduce_writes,
                &uniforms, probe_count);
        if (good)
            good = probe_wavefront_copy_to_readback(
                r, cmd, &counters, counter_readback,
                4u * sizeof(uint32_t));
        ...
        const uint32_t *values = r->core.MapBuffer(
            counter_readback, 0, 4u * sizeof(uint32_t));
        if (!values) {
            good = false;
            break;
        }
        ...
        if (progress && !progress(completed, progress_total, active))
            good = false;
        SDL_Log("B: probe wavefront %u/%u spp | %u active probes",
                completed, PROBE_MAX_SAMPLES, active);
    }
    ...
    if (good) {
        ...
        if (good) {
            good = probe_wavefront_copy_to_readback(
                r, cmd, &coefficients, output_readback, output_bytes);
            if (good) good = submit_commands(r, allocator, cmd);
            else abort_commands(r, allocator, cmd);
        }
    }
    ...
    if (good) {
        const float (*values)[4] = r->core.MapBuffer(
            output_readback, 0, output_bytes);
        if (!values) good = false;
        else {
            for (uint32_t i = 0; i < probe_count; ++i) {
                for (uint32_t coefficient = 0; coefficient < 9u; ++coefficient)
                    memcpy(grid->probes[i].coefficients[coefficient],
                           values[i * 9u + coefficient], sizeof(float[4]));
                grid->probes[i].position[3] = values[i * 9u][3];
            }
...
static bool dispatch_one(fx_state *fx, NriCommandBuffer *cmd,
                         NriPipeline *pipeline, NriTexture *source,
                         NriTexture *destination, const void *uniforms,
...
static bool fx_ensure(fx_state *fx, Uint32 width, Uint32 height) {
    ...
    fx->normal_depth = create_texture(
        r, NriFormat_RGBA16_SFLOAT, rt, width, height);
    fx->ao = create_texture(
        r, NriFormat_RGBA16_SFLOAT, compute,
        fx->ao_width, fx->ao_height);
    fx->bloom_a = create_texture(
        r, NriFormat_RGBA16_SFLOAT, compute,
        fx->ao_width, fx->ao_height);
    fx->bloom_b = create_texture(
        r, NriFormat_RGBA16_SFLOAT, compute,
        fx->ao_width, fx->ao_height);
    fx->volume = create_texture(
        r, NriFormat_RGBA16_SFLOAT, compute,
        fx->ao_width, fx->ao_height);
    fx->lit = create_texture(
        r, NriFormat_RGBA16_SFLOAT, compute,
        width, height);
...
static bool fx_volume(fx_state *fx, NriCommandBuffer *cmd,
                      NriBuffer *probes, NriBuffer *beams,
                      const probe_grid *grid,
                      const beam_grid *beam_grid,
...
static bool fx_apply_base(fx_state *fx, NriCommandBuffer *cmd,
                          NriTexture *swap, float tan_half_fov, float aspect) {
...
bool draw_frame(renderer *r, const render_frame *frame) {
    ...
    NriCommandBuffer *cmd = NULL;
    if (!begin_frame_commands(r, &queued_frame, &cmd))
        goto failed_frame;
    ...
    if (!submit_frame(r, queued_frame, cmd, swap_index))
        return false;
...
bool bake_worker_init(renderer *r) {
    ...
    r->lightmap_texture = create_texture(
        r,
        NriFormat_RGBA16_SFLOAT,
        NriTextureUsageBits_SHADER_RESOURCE,
        1, 1);
