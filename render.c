...
bool r_rebake_current_scene(renderer *r, const mesh *m, const gltf_scene *visual,
                            const lightmap *lm, const char *path,
...
bool r_build_scene(renderer *r, const mesh *m, const gltf_scene *visual,
                   const lightmap *lm) {
    if (!r || !m || !visual || !lm || !visual->vertex_count ||
        visual->vertex_count % 3u || visual->vertex_count / 3u != m->faces.count ||
        !visual->material_count || !lm->uvs) {
        SDL_Log("render/lightmap geometry mismatch");
        return false;
    }

    r->target = m->bounds.center;
    r->scene_radius = fmaxf(m->bounds.extents.x,
                            fmaxf(m->bounds.extents.y, m->bounds.extents.z));
    ...
    for (uint32_t material = 0; material < visual->material_count; ++material) {
        ...
        for (size_t triangle = 0; triangle < triangle_count; ++triangle) {
            ...
            if (!push_surface(r, &v[0], uv[0]) ||
                !push_surface(r, &v[1], uv[1]) ||
                !push_surface(r, &v[2], uv[2])) return false;
        }
    ...
    for (size_t triangle = 0; triangle < triangle_count; ++triangle) {
        const gltf_vertex *v = &visual->vertices[triangle * 3u];
        if (!add_wire_triangle(r, v[0].position, v[1].position, v[2].position, wire))
            return false;
    }
    ...
    r->has_bake = false;
    SDL_Log("materials: %u | material draw ranges: %u | embedded images: %u",
            r->material_count, r->draw_count, r->image_texture_count);
    return true;
}
...
void r_event(renderer *r, const SDL_Event *event) {
    ...
    switch (event->type) {
        ...
        case SDL_EVENT_MOUSE_WHEEL:
            ...
            if (r->distance < r->scene_radius * 0.05f)
                r->distance = r->scene_radius * 0.05f;
            if (r->distance > r->scene_radius * 20.0f)
                r->distance = r->scene_radius * 20.0f;
            break;

        case SDL_EVENT_KEY_DOWN:
            if (!event->key.repeat && event->key.key == SDLK_TAB)
                r->show_debug = !r->show_debug;
            if (!event->key.repeat && event->key.key == SDLK_F5)
                r->show_volume = !r->show_volume;
            if (!event->key.repeat &&
...
bool r_draw(renderer *r) {
    ...
    const vec3 eye = v3(
        r->target.x + r->distance * cp * cosf(r->yaw),
        r->target.y + r->distance * sinf(r->pitch),
        r->target.z + r->distance * cp * sinf(r->yaw));
