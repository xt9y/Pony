...
static void unpack_grid(probe_grid *grid, const uint32_t dims[3],
...
bool cache_write(const char *path, uint64_t scene_hash, uint64_t layout_hash,
    ...
    SDL_Log("B: cache hashing took %.2f ms", (double)(SDL_GetPerformanceCounter() - hash_started) * 1000.0 /
    ...
    if (fclose(file) != 0) good = false;
    if (good) SDL_Log("B: cache file write took %.2f ms", (double)(SDL_GetPerformanceCounter() - write_started) * 1000.0 /
                      (double)SDL_GetPerformanceFrequency());
